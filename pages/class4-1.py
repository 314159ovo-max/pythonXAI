...
#dict的key必须是不可變的資料型態，如string、int、float等
#dict的value可以是任何的資料类型
#dict的key-value是透過冒號來連接,key:value
#dict的key-value是透過逗號來分隔
d={"a":1,"b":2,"c":3}

#取得dict的key
print(d.keys())#dict_key(['a', 'b', 'c'])
for key in d.keys():
    print(key)
...
#取得dict的key-value
print(d.items())#dict_key([('a',1),('b',2),('c',3)])
for key,value in d.items():
    print(key,value)

#新增/修改dict的key-value
d["d"]=4
print(d)#dict_key(['a':1, 'b':2, 'c':3, 'd':4])
d["a"]=5
print(d)#dict_key(['a':1, 'b':2, 'c':3, 'd':4])
