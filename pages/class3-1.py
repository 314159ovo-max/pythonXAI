...
col2.button("按鈕2",key="btn2")#在col2中建立一個按鈕類似st.button("案有2")

#2columns,可以用比例來設置每個column的寬度,將比例放到list中
col1,col2=st.columns([1, 2])
col1.button("按鈕1",key="btn3")#在col1中建立一個按鈕類似st.button("按鈕1")
col2.button("按鈕2",key="btn4")#在col2中建立一個按鈕類似st.button("案有2")
...
with col1:#在col1使用with語句放更多東西
    ...
    st.write("這是col1")#在cold1建立一個文字
with col2: #在col2使用with 語句放更多東西
    st.button("按鈕2",key="btn9")#在col2中建立一個按鈕
    st.write("這是col2")#在cold2建立一個文字

#多個columns可以使用for迴圈來建立
cols=st.columns(4) #q4columns, clos[0]...clos[3]
for i in range(len(cols)):
    with cols[i]:
        st.button(f"按鈕{i+1}",key=f"btn{i+10}")#在col1中建立一個按鈕類似
#st.ballons()#在col[I]中建立一個氣球
